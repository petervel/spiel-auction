...
CREATE TABLE `User` (
    ...
    `bggUsername` VARCHAR(191) NULL,
    `accessLevel` ENUM('ADMIN', 'NORMAL', 'MODERATOR') NOT NULL DEFAULT 'NORMAL',
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `googleId` VARCHAR(191) NULL,
    `currentUserFairId` INTEGER NULL,

    UNIQUE INDEX `User_email_key`(`email`),
    UNIQUE INDEX `User_googleId_key`(`googleId`),
    INDEX `User_currentUserFairId_fkey`(`currentUserFairId`),
    PRIMARY KEY (`id`)
...
CREATE TABLE `UserFair` (
    ...
    INDEX `UserFair_fairId_fkey`(`fairId`),
    UNIQUE INDEX `UserFair_userId_fairId_key`(`userId`, `fairId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- CreateTable
CREATE TABLE `UserStarredItem` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `userId` INTEGER NOT NULL,
    `itemId` INTEGER NOT NULL,

    INDEX `UserStarredItem_itemId_fkey`(`itemId`),
    UNIQUE INDEX `UserStarredItem_userId_itemId_key`(`userId`, `itemId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `Fair` (
    ...
    UNIQUE INDEX `Fair_geeklistId_key`(`geeklistId`),
...
-- AddForeignKey
ALTER TABLE `UserStarredItem` ADD CONSTRAINT `UserStarredItem_itemId_fkey` FOREIGN KEY (`itemId`) REFERENCES `Item`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `UserStarredItem` ADD CONSTRAINT `UserStarredItem_userId_fkey` FOREIGN KEY (`userId`) REFERENCES `User`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
